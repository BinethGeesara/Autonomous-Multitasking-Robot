...
const unsigned long frontSensorDebounceDelay = 100; //  debounce delay 
unsigned long frontSensorDebounceStart = 0;
bool frontSensorsOnLineConfirmed = false;
bool hasExecutedWallFollow = false;
...
RobotState previousState = LINE_FOLLOW;  // Declaring previousState globally
...
void turnLeft180() {
  ...
  const unsigned long maxWaitTime = 1500;  // time for 180-degree turn
...
void executePick() {
  ...
  // Performing 180-degree turn
  turnLeft180();
  
  // Adding a small delay to stabilize
  delay(200);
  
  // Clearing any sensor flags
  ...
  detachInterrupt(digitalPinToInterrupt(FRONT_IR_DIGITAL));
  currentState = LINE_FOLLOW;
}
void executePlace() {
  // First stop motors for 0.5 second
  motor1.drive(0);
  motor2.drive(0);
  delay(500);
  motor1.drive(-baseSpeed);
...
void executeWallFollow() {
  ...
  currentState = LINE_FOLLOW;  // Small delay to avoid excessive looping
  
}
void turnLeft() {
  motor1.drive(-65);  // Left motor backward
  motor2.drive(65);   // Right motor forward
...
void turnRight() {
  motor1.drive(65);   // Left motor forward
  motor2.drive(-65);  // Right motor backward
  delay(0);           // Shortened delay for a quicker turn
  motor1.drive(0);
  motor2.drive(0);
}
void updateState() {
  ...
  if (!frontSensorsOnLine && analogRead(FRONT_IR_ANALOG) < 800) {
    currentState = WALL_FOLLOW;
  }
  if (previousState == WALL_FOLLOW && currentState != WALL_FOLLOW) {
    hasExecutedWallFollow = false;  // Allow re-entry into WALL_FOLLOW in the future
...
void executeState() {
    switch (currentState) {
        ...
        case WALL_FOLLOW:
            if (!hasExecutedWallFollow) {
                executeWallFollow();
                hasExecutedWallFollow = true;  // Ensure it runs only once
            }
            break;
    }
}
